...
job_dir=$base_dir/results/11_downsampling/2_extract_mutational_signatures_from_vcfs/${current_date}/scripts/jobs/; mkdir -p $job_dir

# create output dirs if they dont exist
output_dir=$base_dir/results/11_downsampling/2_extract_mutational_signatures_from_vcfs/${current_date}/results/matrices/sv; mkdir -p $output_dir

cd $job_dir

# Rscript path
R_SCRIPT=$base_dir/code/11_downsampling/2_extract_mutational_signatures_from_vcfs/extract_sv_type_load.R
...
zcat $manifest_path | tail -n +2 | while read $manifest_column_names; do
	...
	job_file=$job_dir/extract_sv_${sample}.job
	...
	Rscript $R_SCRIPT $sample $sv_vis_data $output_dir && touch ${job_file}.done
	" > $job_file
