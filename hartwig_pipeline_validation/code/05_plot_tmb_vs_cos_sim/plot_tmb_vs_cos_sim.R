...
# This script depends on the four scripts that each plot and save the output plots to a specified folder:
...
### Input
# - The cosine similarity and mutations burden tables you generated in step 4 for all the mutation types.
# - metadata.tsv file.
...
# create output dir
output_dir <- paste0(output_dir, '/05_plot_tmb_vs_cos_sim/', current_date, '/results/plots')
...
snv_total_mut_burden <- read_tsv(paste0(base_dir, '/results/04_calculate_total_mutational_burden/', tmb_input_results_date, '/results/snv_total_mutational_burden.tsv'))
dbs_total_mut_burden <- read_tsv(paste0(base_dir, '/results/04_calculate_total_mutational_burden/', tmb_input_results_date, '/results/dbs_total_mutational_burden.tsv'))
indel_total_mut_burden <- read_tsv(paste0(base_dir, '/results/04_calculate_total_mutational_burden/', tmb_input_results_date, '/results/indel_total_mutational_burden.tsv')) %>%
  mutate(indel_pcawg_total_mutational_burden = as.numeric(indel_pcawg_total_mutational_burden))
sv_total_mut_burden <- read_tsv(paste0(base_dir, '/results/04_calculate_total_mutational_burden/', tmb_input_results_date, '/results/sv_total_mutational_burden.tsv'))
sv_total_mut_burden_len_cutoff <- read_tsv(paste0(base_dir, '/results/04_calculate_total_mutational_burden/', tmb_input_results_date, '/results/sv_total_mutational_burden_len_cutoff.tsv')) %>%
  mutate(sv_pcawg_total_mutational_burden_len_cutoff = as.numeric(sv_pcawg_total_mutational_burden_len_cutoff))

# ------------------ COSINE SIMILARITY

snv_cosine_similarity <- read_tsv(paste0(base_dir, '/results/04_calculate_cosine_similarity/', cos_input_results_date, '/results/snv_cosine_similarity.tsv'),
                              col_names = TRUE) %>%
  mutate(snv_cosine_similarity = as.numeric(snv_cosine_similarity))
dbs_cosine_similarity <- read_tsv(paste0(base_dir, '/results/04_calculate_cosine_similarity/', cos_input_results_date, '/results/dbs_cosine_similarity.tsv'),
                                  col_names = TRUE) %>%
  mutate(dbs_cosine_similarity = as.numeric(dbs_cosine_similarity))
indel_cosine_similarity <- read_tsv(paste0(base_dir, '/results/04_calculate_cosine_similarity/', cos_input_results_date, '/results/indel_cosine_similarity.tsv'),
                                  col_names = TRUE) %>%
  mutate(indel_cosine_similarity = as.numeric(indel_cosine_similarity))
sv_cosine_similarity <- read_tsv(paste0(base_dir, '/results/04_calculate_cosine_similarity/', cos_input_results_date, '/results/sv_cosine_similarity.tsv'),
                                  col_names = TRUE) %>%
  mutate(sv_cosine_similarity = as.numeric(sv_cosine_similarity))
sv_cosine_similarity_len_cufoff <- read_tsv(paste0(base_dir, '/results/04_calculate_cosine_similarity/', cos_input_results_date, '/results/sv_cosine_similarity_len_cutoff.tsv'),
                                 col_names = TRUE) %>%
...
source(paste0(here::here(), '/code/05_plot_tmb_vs_cos_sim/boxplot.R'))

############################## SLOPE PLOT ############################################

# source(paste0(here::here(), '/code/05_plot_tmb_vs_cos_sim/slope_plot.R'))

############################## RATIO PLOT ############################################

source(paste0(here::here(), '/code/05_plot_tmb_vs_cos_sim/ratio_plot.R'))

############################## WATERFALL PLOT ########################################

source(paste0(here::here(), '/code/05_plot_tmb_vs_cos_sim/waterfall_plot.R'))
