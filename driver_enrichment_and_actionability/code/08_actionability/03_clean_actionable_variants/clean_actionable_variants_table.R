...
### Description
# input: actionable variants table that was parsed from VCF files by parse_actionable_variants.R script (step 8.2..
# This script cleans the actionable variants table parsed from VCFs for actionable variants that are present in the
# Hartwig actionability table.

### Input
# metadata.tsv
# combined purity file for all samples (PURPLE output)
# actionability table generated in step 8.2.
# combined actionable variants from VCF table generated in step 8.1.
# LINX drivers table generated in step 2.
# LINX fusions table generated in step 2.

### Output
# A cleaned up actionable variants table that contain 3 columns: sample_id, (mutated) gene, actionable variant
...
actionable_variants_results_date <- '2022_01_26' # date name of your output folder you generated in step 8.1
linx_drivers_results_date <- '2021_12_03' # date name of your LINX driver output folder you generated in step 2
fusion_results_date <- '2021_12_03' # date name of your LINX fusion output folder you generated in step 2
purity_results_date <- '2021_12_03' # date name of your purity folder
...
purity <- read_tsv(paste0(base_dir, '/data/processed/metadata/',
...
actionable_variants <- read_tsv(paste0(base_dir, '/results/08_actionability/',
                                       actionable_variants_results_date ,'/results/actionable_variants_prefilter.tsv.gz'),
                         col_names = TRUE)
...
linx_drivers <- read_tsv(file = paste0(base_dir, '/results/02_combine_driver_catalogs/',
                                       linx_drivers_results_date ,'/driver/results/linx_drivers.tsv'))
...
linx_fusions_high <- read_tsv(paste0(base_dir, '/results/02_combine_driver_catalogs/',
                                     fusion_results_date ,'/fusion/results/linx_fusions.tsv'),
                         col_names = TRUE)
...
# only keep samples that are in the metadata file
actionable_variants_clean <- actionable_variants_clean %>%
  semi_join(metadata, by = 'sample_id')

# write final clean combined table to disk
write_tsv(actionable_variants_clean, file = paste0(base_dir, '/results/08_actionability/',
                                                   actionable_variants_results_date ,'/results/actionable_variants.tsv'))
