...
linx_drivers <- read_tsv(paste0(base_dir, '/processed/driver/', driver_results_date ,'/linx_drivers.tsv'),
                         col_names = TRUE) %>%
  filter(gene == mutated_driver) %>%
  distinct(sample_id, gene)

# join the driver gene table to the metadata table, then add a new column 'is_mutated'
metadata <- metadata %>%
  left_join(linx_drivers, by = 'sample_id') %>%
...
metadata_wider_is_mutated_combined <- metadata_wider_is_mutated_true %>%
  ...
  rename(metastatic_is_mutated = 'is_mutated_true_Hartwig',
         metastatic_non_mutated = 'is_mutated_false_Hartwig',
...
# calculate fisher p-value, FDR q-value, odds ratio and cramers v + add these as separate columns
metadata_wider_is_mutated_combined$fisher_pval <- fisherTest.matrix(metadata_wider_is_mutated_combined_mat)
metadata_wider_is_mutated_combined$fisher_pval_adj <- p.adjust(metadata_wider_is_mutated_combined$fisher_pval, method = 'fdr')
...
metadata_wider_is_mutated_combined_plot <- metadata_wider_is_mutated_combined %>%
  ...
  mutate(significance_level = if_else(fisher_pval_adj < q_val_cutoff & stadium_is_mutated == 'primary_is_mutated_fraction', '*', NA_character_))
